...
export default function PodOrdersList({ orders, newOrderPath = '/print-on-demand' }) {
  ...
  if (!list.length) {
    return (
      <section className="meus-pedidos-empty" aria-label="Sem pedidos de mangá físico">
        <p>Você ainda não tem pedidos de mangá físico registrados.</p>
        <p className="meus-pedidos-empty__hint">
          Encomendas para vender pela plataforma, produção para você ou modo vitrine na loja entram por aqui.
        </p>
        <Link className="meus-pedidos-cta" to={newOrderPath}>
          Novo pedido de mangá físico
        </Link>
      </section>
    );
  }
  ...
  return (
    <section className="meus-pedidos-pod-list" aria-label="Pedidos de mangá físico">
            ...
            <li key={id}>
              <article className="meus-pedidos-pod-card">
                <div className="meus-pedidos-pod-card__main">
                  <div>
                    <h3>Pedido físico #{shortPodOrderId(id)}</h3>
                ...
                {expanded ? (
                  <div className="meus-pedidos-pod-card__detail">
                    ...
                    <p className="meus-pedidos-pod-card__hint">
                      Atualizações também aparecem nas <Link to="/perfil">notificações da conta</Link>.
                    </p>
                  </div>
                ) : null}
              </article>
